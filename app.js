...
function setupEventListeners() {
    ...
    // Image Save Event Listeners
    const btnSaveResultsImg = document.getElementById('btn-save-results-img');
    if (btnSaveResultsImg) {
        btnSaveResultsImg.addEventListener('click', () => {
            saveElementAsImage('results-layout-container', 'dps_comparison_results.png', btnSaveResultsImg);
        });
    }

    const btnSaveRankingsImg = document.getElementById('btn-save-rankings-img');
    if (btnSaveRankingsImg) {
        btnSaveRankingsImg.addEventListener('click', () => {
            saveElementAsImage('rankings-section-container', 'class_rankings.png', btnSaveRankingsImg);
        });
    }

    const btnSaveCharResultsImg = document.getElementById('btn-save-char-results-img');
    if (btnSaveCharResultsImg) {
        btnSaveCharResultsImg.addEventListener('click', () => {
            saveElementAsImage('char-results-panel', 'character_dps_comparison_results.png', btnSaveCharResultsImg);
        });
    }
}
...
function calculateAndRender() {
    ...
    // Render metadata banner at the top
    renderMetaBanner('results-meta-banner');
...
// Helpers for displaying raid/difficulty/patch metadata in the results banner
function getBossDisplayName(apiName) {
    for (const raid of RAID_METADATA) {
        const found = raid.bosses.find(b => b.apiName === apiName);
...
function getDifficultyDisplayName(diff) {
    const translation = {
        "Nightmare": "나이트메어 (Nightmare)",
        "Hard": "하드 (Hard)",
        "Normal": "노말 (Normal)",
        "The First": "더 퍼스트 (The First)",
        "Extreme Hard": "익스트림 하드 (Extreme Hard)",
        "Extreme Normal": "익스트림 노말 (Extreme Normal)",
        "Level 1": "1단계 (Level 1)",
        "Level 2": "2단계 (Level 2)",
        "Level 3": "3단계 (Level 3)"
    };
    return translation[diff] || diff;
}

function getPatchDisplayName(patch) {
    const translation = {
        "jun26": "최신 패치 (jun26)",
        "mar26": "이전 패치 (mar26)"
    };
    return translation[patch] || patch;
}

function renderMetaBanner(bannerId) {
    const banner = document.getElementById(bannerId);
    if (!banner) return;
    
    const bossText = getBossDisplayName(currentBoss);
    const diffText = getDifficultyDisplayName(currentDifficulty);
    const patchText = getPatchDisplayName(currentPatch);
    
    banner.innerHTML = `
        <div class="results-meta-item">
            <span class="results-meta-label">⚔️ 레이드 / 보스:</span>
            <span class="results-meta-value">${bossText}</span>
        </div>
        <div class="results-meta-divider"></div>
        <div class="results-meta-item">
            <span class="results-meta-label">🛡️ 난이도:</span>
            <span class="results-meta-value">${diffText}</span>
        </div>
        <div class="results-meta-divider"></div>
        <div class="results-meta-item">
            <span class="results-meta-label">📅 패치 버전:</span>
            <span class="results-meta-value">${patchText}</span>
        </div>
    `;
}
...
function renderRankingsTable() {
    // Render metadata banner at the top of the rankings table
    renderMetaBanner('rankings-meta-banner');
...
function calculateAndRenderCharacterDPS() {
    ...
    // Render metadata banner at the top of character results
    renderMetaBanner('char-results-meta-banner');
...
// Helper: Save HTML Element as Image using html2canvas
function saveElementAsImage(elementId, filename, buttonEl) {
    console.log(`[SaveImage] saveElementAsImage called for #${elementId} -> ${filename}`);
    const element = document.getElementById(elementId);
    if (!element) {
        console.error(`[SaveImage] Target element #${elementId} not found in DOM!`);
        alert(`저장할 대상을 찾을 수 없습니다: #${elementId}`);
        return;
    }
    
    const originalText = buttonEl.innerHTML;
    console.log(`[SaveImage] Target element found. Original button text: "${originalText.trim()}"`);
    
    if (typeof html2canvas === 'undefined') {
        console.error('[SaveImage] html2canvas library is undefined!');
        alert('이미지 저장 라이브러리(html2canvas)를 로드할 수 없습니다. 인터넷 연결 상태를 확인해 주세요.');
        return;
    }
    
    console.log('[SaveImage] html2canvas is loaded. Disabling button and changing text to loading...');
    ...
    const options = {
        ...
        ignoreElements: (el) => {
            // Ignore any "Save Image" buttons during screenshot rendering
            return el && (el.id === buttonEl.id || (el.id && el.id.startsWith('btn-save-')));
        }
    };
    
    console.log('[SaveImage] Starting html2canvas render...');
    html2canvas(element, options).then(canvas => {
        console.log('[SaveImage] html2canvas render completed successfully. Creating blob...');
        
        canvas.toBlob(blob => {
            if (!blob) {
                console.error('[SaveImage] Failed to create blob from canvas!');
            ...
            console.log('[SaveImage] Blob created. Creating download link...');
            ...
            // Clean up resources after short delay
            setTimeout(() => {
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
            }, 100);
            
            console.log('[SaveImage] Download link triggered. Restoring button state.');
            buttonEl.disabled = false;
            buttonEl.innerHTML = originalText;
        }, 'image/png');
        
    }).catch(err => {
        console.error('[SaveImage] html2canvas render failed:', err);
