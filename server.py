...
# In-memory cache for live database
db_cache = {
    "data": None,
    "timestamp": 0
}
...
# Deserialize SvelteKit nodes data
...
# Dynamically scrape SvelteKit build hashes from lostark.bible
# Dynamically scrape SvelteKit build hashes from lostark.bible
def get_live_hashes():
    ...
    html = resp.text
    # Extract links matching JS files
    ...
    # SvelteKit entry/app.*.js contains references to all other node and chunk files
    extended_urls = []
    for url in js_urls:
    ...
    # Priority sorting: check nodes/59. first, then nodes/5.
    def url_priority(u):
...
# Fetch live relative stats from API
def fetch_live_stats():
    cp_hash = get_live_hashes()
    if not cp_hash:
        raise Exception("Failed to resolve SvelteKit remote build hashes")
    ...
    # Fetch all 4 dpsTypes in parallel
    print("Fetching live stats from lostark.bible in parallel...")
    ...
    # We use 'ndps' list as base because it is the most common, but we fallback
    ...
    # Merge values across all dpsTypes
...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed_path.path == "/api/live-stats":
            # Check cache
            current_time = time.time()
            if db_cache["data"] and (current_time - db_cache["timestamp"] < CACHE_TTL):
                print("Serving stats from cache...")
                self.send_json(db_cache["data"])
                return
                
            try:
                combined_db = fetch_live_stats()
                db_cache["data"] = combined_db
                db_cache["timestamp"] = current_time
                self.send_json(combined_db)
            except Exception as e:
                print("Error loading live stats:", e)
                # Fallback to local database.js
                fallback_db_path = "database.js"
                if os.path.exists(fallback_db_path):
                    print("API failed. Falling back to local database.js...")
                    try:
                        ...
                        if start != -1 and end != -1:
                            local_db = json.loads(content[start:end])
                            self.send_json(local_db)
                            return
                    except Exception:
                        pass
...
def run():
    # Force absolute paths in simple HTTP serving to match D:\Desktop\로스트아크_성능_비교기
